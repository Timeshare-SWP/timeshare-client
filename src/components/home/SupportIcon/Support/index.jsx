...
import { createSupport } from '../../../../redux/features/supportSlice';
import toast from 'react-hot-toast';

const Support = ({ onClose }) => {

    const { userDecode } = useContext(AuthContext);
    const [name, setName] = useState(userDecode?.fullName ? userDecode?.fullName : '')
    const [email, setEmail] = useState(userDecode?.email || '')
    const [msgSupport, setMsgSupport] = useState('')
    ...
    const sendMessage = async () => {
        ...
        if (email.trim() === '') {
            setError(prevState => ({
                ...prevState,
                email: 'Hãy nhập địa chỉ email của bạn!'
            }));
        }
        ...
        if (msgSupport.trim() === '') {
            setError(prevState => ({
                ...prevState,
                msgSupport: 'Hãy nhập điều bạn cần hỗ trợ!'
            }));
        }

        if (name === '' || email === '' || msgSupport === '' || !isValidEmail((email.trim()))) {
            return;
        }
        ...
        const data = {
            email: email,
            support_content: msgSupport,
            support_type: "Search Support",
        };

        try {
            await dispatch(createSupport(data));
    ...
    return (
        <div className='feedback-container'>
            <div className="container d-flex justify-content-center align-items-center">
                ...
                <div>
                    <h1 className="title text-center mb-4">Gửi hỗ trợ đến chúng tôi</h1>
                    ...
                    <div className="form-group message">
                        <textarea id="formMessage"
                            ...
                            className="form-control form-control-lg" rows="7" placeholder="Lời nhắn"></textarea>
                    </div>
                    {error.msgSupport && <p className="error-message">{error.msgSupport}</p>}


                    <div className="text-center">
                        <button onClick={sendMessage} type="submit" className="btn btn-primary" tabIndex="-1">Gửi hỗ trợ</button>
                    </div>
                </div>
